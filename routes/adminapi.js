...
router.post('/login',async (req,res) => {
    try {
        ...
        console.log(data.role,typeof(data.role))
        if(!data) return res.send("Enter a valid email address")
            ...
            }else return res.send({message: "You are not authorised to access this"})
        }        
    } catch (error){
...
//     //     email: req.body.email
//     // })
//     let data = await Admin.find({_id: req.cookies.id})
...
//     let data = await Admin.findById(req.cookies.id)
//     console.log(data)
